...
namespace Google.Impl
{
    ...
    internal class NativeFuture : BaseObject, FutureAPIImpl<GoogleSignInUser>
    {
        ...
        public bool Pending
        {
            get { return GoogleSignInImpl.GoogleSignIn_Pending(SelfPtr()); }
        }
        ...
        public GoogleSignInUser Result
        {
            get
            {
                ...
                if (ptr != IntPtr.Zero)
                {
                    ...
                    user.DisplayName = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetDisplayName(
                                userPtr,
                                out_string,
                                out_size
                            )
                    );
                    user.Email = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetEmail(userPtr, out_string, out_size)
                    );

                    user.FamilyName = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetFamilyName(
                                userPtr,
                                out_string,
                                out_size
                            )
                    ...
                    user.GivenName = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetGivenName(
                                userPtr,
                                out_string,
                                out_size
                            )
                    ...
                    user.IdToken = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetIdToken(userPtr, out_string, out_size)
                    );

                    user.AuthCode = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetServerAuthCode(
                                userPtr,
                                out_string,
                                out_size
                            )
                    ...
                    string url = OutParamsToString(
                        (out_string, out_size) =>
                            GoogleSignInImpl.GoogleSignIn_GetImageUrl(userPtr, out_string, out_size)
                    );
                    if (url.Length > 0)
                    {
        ...
        {
            get { return (GoogleSignInStatusCode)GoogleSignInImpl.GoogleSignIn_Status(SelfPtr()); }
        }
    }
}
